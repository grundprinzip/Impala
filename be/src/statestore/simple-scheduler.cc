...
Status SimpleScheduler::Init() {
  ...
  if (metrics_ != NULL) {
    total_assignments_ = metrics_->AddCounter(ASSIGNMENTS_KEY, 0LL);
    total_local_assignments_ = metrics_->AddCounter(LOCAL_ASSIGNMENTS_KEY, 0LL);
...
Status SimpleScheduler::GetBackend(const TNetworkAddress& data_location,
  ...
  *backend = entry->second.front();
  ...
  if (metrics_ != NULL) {
    ...
    if (local_assignment) {
      total_local_assignments_->Increment(1LL);
    }
  }
...
Status SimpleScheduler::Schedule(Coordinator* coord, QuerySchedule* schedule) {
  ...
  schedule->set_num_hosts(max(num_backends_metric_->value(), 1LL));
