...
TEST(CountersTest, DerivedCounters) {
  ...
  // set to 1 sec
  ticks_counter->Set(1000LL * 1000LL * 1000LL);
  ...
  bytes_counter->Set(10LL);
  EXPECT_EQ(throughput_counter->value(), 10);
  bytes_counter->Set(20LL);
...
TEST(CountersTest, AverageSetCounters) {
  ...
  bytes_1_counter->Set(10LL);
  RuntimeProfile::AveragedCounter bytes_avg(TUnit::BYTES);
  bytes_avg.UpdateCounter(bytes_1_counter);
  // Avg of 10LL
  EXPECT_EQ(bytes_avg.value(), 10LL);
  bytes_1_counter->Set(20LL);
  bytes_avg.UpdateCounter(bytes_1_counter);
  // Avg of 20L
  EXPECT_EQ(bytes_avg.value(), 20LL);
  bytes_2_counter->Set(40LL);
  bytes_avg.UpdateCounter(bytes_2_counter);
  // Avg of 20L and 40L
  EXPECT_EQ(bytes_avg.value(), 30LL);
  bytes_2_counter->Set(30LL);
  bytes_avg.UpdateCounter(bytes_2_counter);
  // Avg of 20L and 30LL
  EXPECT_EQ(bytes_avg.value(), 25LL);
...
TEST(CountersTest, RateCounters) {
  ...
  bytes_counter->Set(100LL * 1024LL * 1024LL);
